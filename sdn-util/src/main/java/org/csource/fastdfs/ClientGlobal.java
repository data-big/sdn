...
public class ClientGlobal {
	...
	*/
	public static void init(String conf_filename) throws FileNotFoundException, IOException, MyException {
		...
		String[] parts;
		Properties config = ConfigUtil.getProps(conf_filename);
		g_connect_timeout = Integer
				.valueOf(config.getProperty("connect_timeout", String.valueOf(DEFAULT_CONNECT_TIMEOUT)));
		if (g_connect_timeout < 0) {
			g_connect_timeout = DEFAULT_CONNECT_TIMEOUT;
		}
		g_connect_timeout *= 1000; //millisecond
		g_network_timeout = Integer
				.valueOf(config.getProperty("network_timeout", String.valueOf(DEFAULT_NETWORK_TIMEOUT)));
		if (g_network_timeout < 0) {
			g_network_timeout = DEFAULT_NETWORK_TIMEOUT;
		}
		g_network_timeout *= 1000; //millisecond

		g_charset = config.getProperty("charset");
		if (g_charset == null || g_charset.length() == 0) {
			g_charset = "ISO8859-1";
		}
		szTrackerServers = config.getProperty("tracker_server").split(",");
		...
		g_tracker_http_port = Integer.valueOf(config.getProperty("http.tracker_http_port", String.valueOf(80)));
		g_anti_steal_token = Boolean.valueOf(config.getProperty("http.anti_steal_token", String.valueOf(false)));
		if (g_anti_steal_token) {
			g_secret_key = config.getProperty("http.secret_key");
		}
	}
