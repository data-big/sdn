...
public interface VPNCardDao {
	 ...
	 * @return 成功或失败
	 */
	@Update(value = "UPDATE vpn_card SET invalid=1 WHERE id IN (SELECT id FROM (SELECT id FROM vpn_card WHERE realNumber=#{realNumber} AND invalid=0) AS T)")
				...
				for (String key : param.keySet()) {
					...
					if (!("page".equals(key))) {
						...
						if ("invalid".equals(key)) {
							...
							sql.append(" and ").append("insertDate").append("<=date_add(").append("#{").append(key)
